...
async function addCarHandler(request, response) {
    try {
        const car = await model.addCar(request.body.model, request.body.year, request.body.mileage, request.body.dateBought, request.body.url, request.body.userID);
        response.status(200);
        response.send(car);
...
async function updateCarHandler(request, response) {
    try {
        const car = await model.updateCar(request.body.id, request.body.newModel, request.body.newYear, request.body.newMileage, request.body.newURL, request.body.dateBought);
        response.status(200);
...
async function deleteCarHandler(request, response) {
    try {
        const car = await model.deleteCar(request.params.id);
        response.status(200);
