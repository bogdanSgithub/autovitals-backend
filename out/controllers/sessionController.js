...
async function loginUser(request, response) {
    ...
    try {
        if (username && password && await checkCredentials(username, password)) {
            ...
            const sessionId = createSession(username, numMinutes);
...
function personalAuthenticateUser(request, checkIsAdmin = false) {
    // If this request doesn't have any cookies, that means it isn't authenticated. Return null.
    if (!request.cookies) {
        return null;
    }
...
async function authenticateAdmin(request, checkIsAdmin = false) {
    // If this request doesn't have any cookies, that means it isn't authenticated. Return null.
    if (!request.cookies) {
        return null;
    }
...
function refreshSession(request, response) {
    ...
    const newSessionId = createSession(authenticatedUser.userSession.username, numMinutes);
 ...
 * @returns
 */
function authenticate(request, response, checkIsAdmin = false) {
