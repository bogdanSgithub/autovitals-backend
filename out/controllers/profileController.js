...
async function handleEmailReminder(request, response) {
    let result = "";
    try {
 ...
 * @param response
 */
async function handleGetOneProfile(request, response) {
    logVisit(request, response);
    let result = "";
    try {
        if (!authenticate(request, response))
...
async function handleUpdateProfile(request, response) {
    ...
    const profile = { email: request.body.email, isAdmin: request.body.isAdmin, username: request.body.username, coordinates: request.body.coordinates, emailReminderPreference: request.body.emailReminderPreference };
    try {
        if (!authenticate(request, response, true))
            return;
