 ...
 * @returns
 */
async function addProfile(email, isAdmin, username, coordinates, emailReminderPreference) {
    ...
    try {
        const newProfile = { email: email, isAdmin: isAdmin, username: username, coordinates: coordinates, emailReminderPreference: emailReminderPreference, lastReminderSent: undefined };
 ...
 * @returns
 */
async function getOneProfile(username) {
    if (!profilesCollection) {
        throw new DatabaseError("Collection not initialized");
    }
    try {
        const match = await profilesCollection.findOne({ username });
        if (!match)
 ...
 * @returns
 */
async function updateOneProfile(username, newProfile) {
    if (!profilesCollection) {
        throw new DatabaseError("Collection not initialized");
    }
 ...
 * @returns
 */
async function deleteOneProfile(username) {
    if (!profilesCollection) {
        throw new DatabaseError("Collection not initialized");
    }
...
export { initialize, addProfile, getOneProfile, getAllProfiles, updateOneProfile, deleteOneProfile, close, getCollection };
